...
def get_connection():
    ...
    from .constants import (NOTES_DB)
...
def create_table() -> None:
    ...
    Columns:
      ...
      - updated_at
    """
    from .constants import (NOTES_DB, logging)
...
def center_window(w, wdth, hght, offsetx=0, offsety=0, readjust=False):
    ...
    if readjust:
        x = ((w.winfo_screenwidth() - width) // 2) + width
...
class Askstring(Toplevel):
    ...
    def askstring(self,
        ...
        try:
            ...
            self.lift()
        # Prompt
        label = ctk.CTkLabel(self, text=prompt, wraplength=300)
        label.pack(pady=(15, 5))

        # Entry
        self.entry = ctk.CTkEntry(self, show=show, width=220,
        ...
        ok_btn = ctk.CTkButton(btn_frame, text="OK",
                               command=self.on_ok, width=80)
        ok_btn.pack(side="left", padx=5)

        cancel_btn = ctk.CTkButton(
            btn_frame, text="Cancel", command=self.on_cancel, width=80)
    ...
    def on_ok(self):
        self.result["value"] = self.entry.get()  # type: ignore
        self.destroy()

    def on_cancel(self):
        self.result["value"] = "exit"  # type: ignore
        self.destroy()
